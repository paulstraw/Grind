...
function compileCoffee() {
	async.forEach(files, function(file, callback) {
		...
		file.content = coffeeScript.compile(file.content);

		callback();

		// var coffee = spawn('coffee', ['-c', '-p', file.path], {
		// 	cwd: process.cwd()
		// });

		// coffee.stdout.setEncoding('utf8');
		// coffee.stdout.on('data', function(compiledCoffee) {
		// 	file.content = compiledCoffee;
		// 	callback();
		// });

		// coffee.stderr.on('data', function(data) {
		// 	console.error(clc.red('CoffeeScript ' + data));
		// 	process.exit(1);
		// });

		// coffee.on('exit', function(code) {
		// 	if (code == 127) {
		// 		console.error(clc.red('There was a problem running the `coffee` command. Did CoffeeScript install properly?'));
		// 		process.exit(1);
		// 	}
		// });
	}, function(err) {
